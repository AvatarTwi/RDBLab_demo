...
def tpch_dim(dic_factor):
    tpch_dim_dict = {'Seq Scan': knob_num + basics + len(dic_factor["Seq Scan"]),
                     'Index Scan': num_index + knob_num + basics + 1 + len(dic_factor["Index Scan"]),
                     'Index Only Scan': num_index + knob_num + basics + 1 + len(dic_factor["Index Only Scan"]),
...
def tpch_dim_array(dic_factor):
    tpch_dim_array_dict = {'Seq Scan': [basics,0, 0, len(dic_factor["Seq Scan"]), knob_num, 0],
                           'Index Scan': [basics, num_index, 1,len(dic_factor["Index Scan"]), knob_num, 0],
                           'Index Only Scan': [basics, num_index,1, len(dic_factor["Index Only Scan"]), knob_num,0],
            ...
            if opt.new_data_structure:
                ...
                for i in range(self.num_q):
                    ...
                    TrainEnum, testEnum, TrainTempdata, testTempdata = \
                        train_test_split(enum, temp_data, train_size=int(self.num_sample_per_q),test_size=int(self.num_sample_per_q/4), random_state=opt.random_state)

import pickle
from RDBLab.job_dataset.attr_rel_dict import *
...
def build_attr_queries():
    ...
    with open('attr_queries_med.sql', 'w+') as f:
        for table, cols in rel_attr_list_dict.items():
            ...
            f.write(
                "select \'\"{}\":[\',".format(table) + ",".join(med) + "||\'],\' from RDBLab.{}".format(
                    table) + ";\n")
...
def pickle_dump():
    ...
    # with open('RDBLab/dataset/oltp_dataset/attr_val_dict.pickle', 'wb') as f:

...
from RDBLab.dataset.oltp_dataset.attr_rel_dict import *
import pickle

from RDBLab.dataset.oltp_dataset.cost_factor.cost_factor import cost_factor_main, cost_factor_one2one

import RDBLab.config
basics = 3  # get_basics(plan_dict)
num_rel = len(rel_names)
max_num_attr = max([len(rel_attr_list_dict[key]) for key in rel_attr_list_dict.keys()])
...
with open('RDBLab/dataset/oltp_dataset/attr_val_dict.pickle', 'rb') as f:
...
class OLTPDataset():
    def __init__(self, opt):
        ...
        if opt.new_mid_data:
            ...
            with open(mid_data_dir + '/serialize_knob_mean_range_dict.pickle', 'wb') as f:
            ...
            for grp in all_groups:
                ...
                self.source_list.extend(normalizer.transform(new_samp_dict["feat_vec"]))
            ...
            self.all_dataset = self.make_dataset(self.table_list, self.source_list, self.total_time)
            ...
            self.dim_dict = {
                "min_val": min_val,
                "max_val": max_val,
                "table_list_len": len(all_dicts),
                "maxlen_plan": len(self.source_list[0])
            }

            with open(mid_data_dir + '/serialize_knob_dim_dict.pickle', 'wb') as f:
                pickle.dump(self.dim_dict, f)
            with open(mid_data_dir + '/serialize_knob_grp_idxes.pickle', 'wb') as f:
                pickle.dump(self.grp_idxes, f)
            with open(mid_data_dir + '/serialize_knob_num_grps.pickle', 'wb') as f:
                pickle.dump(self.num_grps, f)
            with open(mid_data_dir + '/serialize_knob_data.pickle', 'wb') as f:
                pickle.dump(self.dataset, f)
            with open(mid_data_dir + '/serialize_knob_test_dataset.pickle', 'wb') as f:
                pickle.dump(self.test_dataset, f)
            with open(mid_data_dir + '/serialize_knob_all_dataset.pickle', 'wb') as f:
                pickle.dump(self.all_dataset, f)
        else:
            with open(mid_data_dir + '/serialize_knob_dim_dict.pickle', 'rb') as f:
                self.dim_dict = pickle.load(f)
            with open(mid_data_dir + '/serialize_knob_grp_idxes.pickle', 'rb') as f:
                self.grp_idxes = pickle.load(f)
            with open(mid_data_dir + '/serialize_knob_num_grps.pickle', 'rb') as f:
                self.num_grps = pickle.load(f)
            with open(mid_data_dir + '/serialize_knob_data.pickle', 'rb') as f:
                self.dataset = pickle.load(f)
                self.datasize = len(self.dataset)
            with open(mid_data_dir + '/serialize_knob_test_dataset.pickle', 'rb') as f:
                self.test_dataset = pickle.load(f)
            with open(mid_data_dir + '/serialize_knob_all_dataset.pickle', 'rb') as f:
                self.all_dataset = pickle.load(f)
            with open(mid_data_dir + '/serialize_knob_mean_range_dict.pickle', 'rb') as f:
    ...
    def make_dataset(self, tables, predicates, labels):
        table_masks = []
        table_tensors = []
        ...
        return dataset.TensorDataset(table_tensors, predicate_tensors, target_tensor, table_masks,
                                     predicate_masks)

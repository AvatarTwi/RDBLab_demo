...
from RDBLab.dataset.postgres_tpch_dataset.cost_factor.cost_factor import cost_factor_main
...
def tpch_dim(dic_factor):
    tpch_dim_dict = {'Seq Scan': knob_num + num_rel + max_num_attr * 3 + basics + len(dic_factor["Seq Scan"]),
                     'Index Scan': num_index + knob_num + num_rel + max_num_attr * 3 + basics + 1 + len(
                         dic_factor["Index Scan"]),
                     'Index Only Scan': num_index + knob_num + num_rel + max_num_attr * 3 + basics + 1 + len(
                         dic_factor["Index Only Scan"]),
                     'Bitmap Heap Scan': knob_num + basics + num_rel + max_num_attr * 3 + 32 + len(
                         dic_factor["Bitmap Heap Scan"]),
                     'Bitmap Index Scan': num_index + knob_num + basics + len(dic_factor["Bitmap Index Scan"]),
                     'Sort': knob_num + basics + len(sort_algos) + 128 + 32 + len(dic_factor["Sort"]),
...
def tpch_dim_array(dic_factor):
    tpch_dim_array_dict = {'Seq Scan': [basics, num_rel + max_num_attr * 3, 0, len(dic_factor["Seq Scan"]), knob_num, 0],
                           'Index Scan': [basics, num_index + num_rel + max_num_attr * 3, 1,
                                          len(dic_factor["Index Scan"]), knob_num, 0],
                           'Index Only Scan': [basics, num_index + num_rel + max_num_attr * 3,1,
                                               len(dic_factor["Index Only Scan"]), knob_num,0],
                           'Bitmap Heap Scan': [basics, num_rel + max_num_attr * 3,0,
                                                len(dic_factor["Bitmap Heap Scan"]), knob_num, 32],
                           'Bitmap Index Scan': [basics, num_index, 0, len(dic_factor["Bitmap Index Scan"]),
                                                 knob_num, 0],
                           'Sort': [basics, 0, len(sort_algos) + 128, len(dic_factor["Sort"]), knob_num,32],
    ...
    def __init__(self, opt):
        ...
        if opt.new_mid_data:
            if opt.new_data_structure:
                ...
                if not os.path.exists(opt.data_structure + opt.cost_factor_dict):
                ...
                for i in range(self.num_q):
                    ...
                    for fname in datapaths[i]:
                        ...
                        [self.add_knobs(data, knobs) for data in data1]

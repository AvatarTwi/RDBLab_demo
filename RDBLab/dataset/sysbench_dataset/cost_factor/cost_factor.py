...
from RDBLab.dataset.sysbench_dataset.cost_factor.cost_factor_linear import linear, linear1
from RDBLab.dataset.sysbench_dataset.cost_factor.cost_factor_nonlinear import nonlinear, nonlinear1
from RDBLab.dataset.sysbench_dataset.cost_factor.cost_factor_smart import smart, smart1
...
def cost_factor_one2one(opt, dir, temp_data, test_size):
    ...
    if 'template' in opt.mid_data_dir:
        ...
        [get_func_input_data(op_data_dic1, data) for data in
         ...
         # data1[:int(pattern_num.findall(opt.mid_data_dir)[-1])]]

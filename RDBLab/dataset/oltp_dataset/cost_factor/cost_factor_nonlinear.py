...
TPCC_GET_COST_FACTOR_DIC = \
    {
        ...
        "ModifyTable": get_default_cost_factor,
        "LockRows": get_default_cost_factor,
        "Result": get_default_cost_factor
    }
...
TPCC_GET_COST_FACTOR_DIC = collections.defaultdict(lambda: func, TPCC_GET_COST_FACTOR_DIC)
...
def nonlinear(op_data_dic, opt):
    ...
    for op in all_dicts:
        ...
        if op in op_data_dic.keys():
            cost_factor_dict[op], error[op] = TPCC_GET_COST_FACTOR_DIC[op](np.array(op_data_dic[op]))
        else:

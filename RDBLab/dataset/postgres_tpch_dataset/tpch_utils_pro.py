...
from RDBLab.dataset.postgres_tpch_dataset.attr_rel_dict import *
from RDBLab.dataset.postgres_tpch_dataset.cost_factor.cost_factor import cost_factor_main, cost_factor_one2one
import RDBLab.config

basics = 9  # get_basics(plan_dict)
...
def tpch_dim_dict(dic_factor):
    tpch_dim_dict = {'Seq Scan': num_rel + max_num_attr * 3 + basics + dic_factor["Seq Scan"],
                     'Index Scan': num_index + num_rel + max_num_attr * 3 + basics + 1 +
                                   dic_factor["Index Scan"],
                     'Index Only Scan': num_index + num_rel + max_num_attr * 3 + basics + 1 +
                                        dic_factor["Index Only Scan"],
                     'Bitmap Heap Scan': basics + num_rel + max_num_attr * 3 + 32 +
                                         dic_factor["Bitmap Heap Scan"],
                     'Bitmap Index Scan': num_index + basics + dic_factor["Bitmap Index Scan"],
                     'Sort': basics + len(sort_algos) + 128 + 32 + dic_factor["Sort"],
                     'Hash': basics + 1 + 32 + dic_factor["Hash"],
                     ...
                     'Aggregate': basics + len(aggreg_strats) + 1 + 32 + dic_factor["Aggregate"],
                     'Nested Loop': 32 * 2 + len(join_types) + basics + dic_factor["Nested Loop"],
                     'Limit': 32 + basics + dic_factor["Limit"],
                     'Subquery Scan': 32 + basics + dic_factor["Subquery Scan"],
                     'Materialize': 32 + basics + dic_factor["Materialize"],
                     'Gather Merge': 32 + basics + dic_factor["Gather Merge"],
                     'Gather': 32 + basics + dic_factor["Gather"],
                     'BitmapAnd': 32 * 2 + basics + dic_factor["BitmapAnd"],
                     'Memoize': 32 + basics + dic_factor["Memoize"]
                     }
    return tpch_dim_dict


with open('RDBLab/dataset/postgres_tpch_dataset/attr_val_dict.pickle', 'rb') as f:
    attr_val_dict = pickle.load(f)


def oprerate_add(basic):
    add = []
    for b in basic:
        add.append(b * math.log(b))
        add.append(b ** 2)
...
def mutiply(basic_vec,inner_vec):
    add = []
    for b in basic:
        add.append(b * math.log(b))
        add.append(b ** 2)
...
def get_basics(plan_dict):
    return oprerate_add([plan_dict['Plan Width'], plan_dict['Plan Rows'], plan_dict['Total Cost']])
...
class PSQLTPCHDataSet():
    def __init__(self, opt):
        ...
        self.num_sample_per_q = int(RDBLab.config.num_per_q[0] * TRAIN_TEST_SPLIT)
        ...
        if opt.new_mid_data:
            ...
            if opt.new_data_structure:
                ...
                for root, dirs, files in os.walk(opt.data_dir):
                    for dir in dirs:
                        ...
                        temp_data = cost_factor_one2one(opt, dir, temp_data,
                                                        max(1, int(RDBLab.config.num_per_q[0] / (20 * 5))))
                ...
                #     train_datas[i] = random.sample(train_datas[i], int(RDBLab.config.num_per_q[0] * 4 / 5))
                #     test_datas[i] = random.sample(test_datas[i], int(RDBLab.config.num_per_q[0] / 5))

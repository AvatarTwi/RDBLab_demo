...
class PSQLTPCHDataSet():
    def __init__(self, opt):
        ...
        self.num_sample_per_q = 1

        dir = opt.data_dir.split('/')[-1]
        ...
        if opt.new_mid_data:
            ...
            for i in range(self.num_q):
                fname = opt.data_dir + "/q" + str(i + 1) + ".json"
                data1 = self.get_all_plans(fname)
                temp_data.append(data1)

            temp_data = cost_factor_one2one(opt, dir, temp_data,1)
            ...
            for i in datas.keys():
                ...
                TrainEnum = enum
                TrainTempdata = temp_data
                testEnum = enum
                testTempdata = temp_data
            ...
            with open(mid_data_dir + '/mean_range_dict' + dir + '.pickle', 'wb') as f:
                pickle.dump(self.mean_range_dict, f)

            self.test_dataset = [self.get_input(grp) for grp in all_groups_test if grp != []]

            with open(mid_data_dir + '/test_dataset' + dir + '.pickle', 'wb') as f:
                pickle.dump(self.test_dataset, f)
        else:
            with open(mid_data_dir + '/test_dataset' + dir + '.pickle', 'rb') as f:

...
class Net_v2():
    def __init__(self, opt, pass_dim_dict):
        ...
        self.filter = True if "filter" in opt.mid_data_dir or "select" in opt.mid_data_dir else False
        if os.path.exists(opt.mid_data_dir.replace("_filter", "").replace("_select", "") + "/save_model_RandomForest" +
            ...
            if "filter" in opt.mid_data_dir or "select" in opt.mid_data_dir:
        ...
        if os.path.exists(opt.mid_data_dir.replace("_select", "").replace("_filter", "") + "/save_model_RandomForest" +
                          "/" + str(opt.batch_size) + "/openfe_values_array.pickle"):
            with open(opt.mid_data_dir.replace("_select", "").replace("_filter", "") + "/save_model_RandomForest" +
    ...
    def _forward_oneQ_batch(self, samp_batch):
        ...
        # input_vec = torch.from_numpy(feat_vec).to(self.device)[:, self.filter_array[samp_batch['node_type']]]
        ...
        if len(input_vec[0]) > 31:
            output_vec = torch.cat((pred_time.reshape(-1, 1), input_vec[:, :30]), axis=1).to(self.device)
        else:
            add_on = torch.zeros(input_vec.shape[0], 31 - input_vec.shape[1]).to(self.device)
            feat_vec = torch.cat((input_vec, add_on), axis=1)
            output_vec = torch.cat((pred_time.reshape(-1, 1), input_vec[:, :30]), axis=1)
    ...
    def calculate_shap(self, eval_dataset):
        ...
        for operator in self.dim_dict:
            ...
            if 'Tree' == filter_type:
                background_data = self.save_X[operator][samp1,:]
                data = self.save_X[operator][samp2,:]
            else:
                background_data = torch.tensor(self.save_X[operator][samp1, :]).to(self.device)
                data = torch.tensor(self.save_X[operator][samp2, :]).to(self.device)
        ...
        for operator in self.dim_dict:
            ...
            try:
                features = ofe.fit(data=train_x,
                                   label=train_y,
                                   tmp_save_path='./openfe_tmp_data_Netv2.feather',
                                   n_jobs=8,
                                   verbose=False)
    ...
    def filterfunc(self, input_vec, filter_array, samp):
        ...
        if samp != 0:
            samp = np.random.choice(np.arange(len(input_vec)), RDBLab.config.num_per_q[-1] * 5, replace=True)
            try:
    ...
    def ofe_transform(self, input_vec, op):
        ...
        ofe = openfe.OpenFE()
        ofe.verbose = False
        ofe.tmp_save_path = './openfe_tmp_data_Netv2.feather'

...
from RDBLab.dataset.oltp_dataset.cost_factor.cost_factor import cost_factor_main
...
num_per_q = 120 * 12 * 12 * 12
op_data_dic = {}

TRAIN_TEST_SPLIT = 0.8


def tpcc_dim(dic_factor):
    tpcc_dim_dict = {'Seq Scan': knob_num + num_rel + max_num_attr * 3 + basics + len(dic_factor["Seq Scan"]),
                     'Index Scan': knob_num + num_index + num_rel + max_num_attr * 3 + basics + 1 + len(
                         dic_factor["Index Scan"]),
                     'Index Only Scan': knob_num + num_index + num_rel + max_num_attr * 3 + basics + 1 + len(
                         dic_factor["Index Only Scan"]),
                     'Bitmap Heap Scan': knob_num + basics + num_rel + max_num_attr * 3 + 32 + len(
                         dic_factor["Bitmap Heap Scan"]),
                     'Bitmap Index Scan': knob_num + num_index + num_rel + max_num_attr * 3 + basics + len(
...
class OLTPDataSet():
    ...
    def normalize(self):  # compute the mean and std vec of each operator
        ...
        def parse_input(data):
            ...
            feat_vec = np.array(
                [np.hstack(
                    (self.input_func[jss["Node Type"]](jss), self.cost_factor_dict[jss["Node Type"]],
                     jss['knob_values']))
                    for jss in data])

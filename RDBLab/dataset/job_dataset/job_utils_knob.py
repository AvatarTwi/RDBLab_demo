...
from RDBLab.dataset.job_dataset.attr_rel_dict import *
import pickle

from RDBLab.dataset.job_dataset.cost_factor.cost_factor import cost_factor_main, cost_factor_one2one
...
def job_dim(dic_factor):
    job_dim_dict = {'Seq Scan': num_rel + max_num_attr * 3 + basics + dic_factor["Seq Scan"],
                    ...
                    'Bitmap Index Scan': num_index + num_rel + max_num_attr * 3 + basics +
                        dic_factor["Bitmap Index Scan"],
                    'Sort': basics + len(sort_algos) + 32 + dic_factor["Sort"],
...
with open('RDBLab/dataset/job_dataset/attr_val_dict.pickle', 'rb') as f:
...
job_GET_INPUT = collections.defaultdict(lambda: get_basics, job_GET_INPUT)
                ...
                for root, dirs, files in os.walk(opt.data_dir):
                    ...
                    for dir in dirs:
                        ...
                        temp_data = cost_factor_one2one(opt, dir, temp_data,
                                                        int(RDBLab.config.num_per_q[0] / 5))

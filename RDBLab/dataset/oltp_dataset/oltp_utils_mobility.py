...
def tpcc_dim(dic_factor):
    tpcc_dim_dict = {'Seq Scan': num_rel + max_num_attr * 3 + basics + len(dic_factor["Seq Scan"]),
                     'Index Scan': num_index + num_rel + max_num_attr * 3 + basics + 1 + len(
                         dic_factor["Index Scan"]),
                     'Index Only Scan': num_index + num_rel + max_num_attr * 3 + basics + 1 + len(
                         dic_factor["Index Only Scan"]),
                     'Bitmap Heap Scan': basics + num_rel + max_num_attr * 3 + 32 + len(
                         dic_factor["Bitmap Heap Scan"]),
                     'Bitmap Index Scan': num_index + num_rel + max_num_attr * 3 + basics + len(
                         dic_factor["Bitmap Index Scan"]),
                     'Sort': basics + len(sort_algos) + 200 + len(dic_factor["Sort"]) + 32,
...
class OLTPDataSet():
    def __init__(self, opt):
        ...
        if opt.new_mid_data:
            ...
            for i in range(self.num_q):
                fname = opt.data_dir + "/" + dir + "/serverlog"
    ...
    def sample_data(self, batch_size):
        ...
        samp = np.random.choice(np.arange(self.datasize), batch_size, replace=True)

...
def tpch_dim(dic_factor):
    tpch_dim_dict = {'Seq Scan': basics + num_rel + max_num_attr * 3 + len(dic_factor["Seq Scan"]),
                     'Index Scan': basics + num_index + num_rel + max_num_attr * 3 + 1 + len(dic_factor["Index Scan"]),
                     'Index Only Scan': basics + num_index + num_rel + max_num_attr * 3 + 1 + len(
                         dic_factor["Index Only Scan"]),
                     'Bitmap Heap Scan': basics + num_rel + max_num_attr * 3 + 32 + len(dic_factor["Bitmap Heap Scan"]),
                     'Bitmap Index Scan': num_index + 3 + len(dic_factor["Bitmap Index Scan"]),
                     'Sort': num_rel * max_num_attr + basics + len(sort_algos) + 32 + len(dic_factor["Sort"]),

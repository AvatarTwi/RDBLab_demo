...
from RDBLab.config import template_num
from RDBLab.dataset.oltp_dataset.cost_factor.cost_factor_linear import linear, linear1
from RDBLab.dataset.oltp_dataset.cost_factor.cost_factor_nonlinear import nonlinear, nonlinear1
from RDBLab.dataset.oltp_dataset.cost_factor.cost_factor_smart import smart, smart1
...
def get_all_plans_template(fname):
    jsonstrs = []
    curr = ""
    prev = None
    prevprev = None
...
def cost_factor_one2one(opt, dir, temp_data):
    ...
    if opt.change == True:
        ...
        [get_func_input_data(op_data_dic1, data) for data in data4cost[:template_num]]
    ...
    if 'template' in opt.mid_data_dir:
        ...
        data1 = get_all_plans_template(fname)

...
from RDBLab.Imp.ImpTreeNode import ImpTreeNode
from RDBLab.R2.deepNN import DeepR2
from RDBLab.R2.tree import TreeR2
from RDBLab.utils.util import get_time
...
class QPPNet():
    ...
    def _forward_oneQ_batch(self, samp_batch):
        '''
        Calcuates the loss for a batch of queries from RDBLab.one query template
    ...
    def buildtree_forward_oneQ_batch(self, samp_batch):
        '''
        Calcuates the loss for a batch of queries from RDBLab.one query template
        ...
        subplans_time = []
        for child_plan_dict in samp_batch['children_plan']:
            child_tree_node, child_output_vec, _ = self.buildtree_forward_oneQ_batch(child_plan_dict)
            if not child_plan_dict['is_subplan']:
                input_vec = torch.cat((input_vec, child_output_vec), axis=1).to(self.device)
                tree_node.child_vec.append(child_output_vec)
                tree_node.child.append(child_tree_node)
            else:
                subplans_time.append(
                    torch.index_select(child_output_vec, 1, torch.zeros(1, dtype=torch.long).to(self.device)).to(
                        self.device))
        ...
        tree_node.output_vec = output_vec
        ...
        tree_node.true_time = torch.from_numpy(samp_batch['total_time']).to(self.device)

        cat_res = torch.cat([pred_time] + subplans_time, axis=1).to(self.device)
        pred_time = torch.sum(cat_res, 1).to(self.device)

        tree_node.pred_time = pred_time
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                print(samp_batch['node_type'], "output_vec: ", output_vec,
                      self.units[samp_batch['node_type']].cpu().state_dict())
            exit(-1)
        return tree_node, output_vec, pred_time

    def collect_forward_oneQ_batch(self, samp_batch, backgroundsamp, datasamp):
        '''
        Calcuates the loss for a batch of queries from RDBLab.one query template
        ...
        subplans_time = []
        for child_plan_dict in samp_batch['children_plan']:
            child_output_vec, _ = self.collect_forward_oneQ_batch(child_plan_dict, backgroundsamp, datasamp)
            if not child_plan_dict['is_subplan']:
                input_vec = torch.cat((input_vec, child_output_vec), axis=1).to(self.device)
            else:
                subplans_time.append(
        ...
        background_data = input_vec[backgroundsamp]
        data = input_vec[datasamp]
        ...
        if 'tree' in self.filter_type:
            for operator in self.dim_dict:
                filter_models[operator] = RandomForestRegressor(max_depth=9, n_estimators=50, warm_start=False,
                                                                random_state=1)
        elif 'net' in self.filter_type:
            filter_models = self.units

        if 'tree' in self.filter_type:
            explainer = shap.TreeExplainer(filter_models[samp_batch['node_type']], background_data)
        elif 'net' in self.filter_type:
            explainer = shap.DeepExplainer(filter_models[samp_batch['node_type']], background_data)
        ...
        if 'tree' in self.filter_type:
            for n in range(n_array.shape[1]):
                sum_array.append(np.sum(np.abs(n_array[:, n])))
        elif 'net' in self.filter_type:
            for n in range(n_array.shape[2]):
                sum_array.append(np.sum(np.abs(n_array[:, :, n])))

        self.shap_values_arrays[samp_batch['node_type']].append(sum_array)
    ...
    def calculate_openfe(self, eval_dataset):
        ...
        for operator in self.dim_dict:
            ...
            samp = 1
            self.save_X[operator] = self.filterfunc(self.save_X[operator], self.save_values_array[operator], samp)
            self.save_y[operator] = self.filterfunc(self.save_y[operator], self.save_values_array[operator], samp)
            train_x = self.transform(self.save_X[operator], samp)
            train_y = self.transform(self.save_y[operator], samp)
            ...
            try:
                features = ofe.fit(data=train_x,
                                   label=train_y,
                                   tmp_save_path='./openfe_tmp_data_QPP.feather',
                                   n_jobs=8,
                                   verbose=False)
    ...
    def calculate_config(self, eval_dataset):
        self.test = True
        self.set_input(eval_dataset)
        self.eval = True
        self._forward(0)

        return self.pred_times

    def calculate_imp(self, eval_dataset):
        ...
        for root in self.roots:
            root.count_importance()

    def calculate_shap_for_each(self, eval_dataset):

        self.shap_values_arrays = {}
        shap_values_arrays_sum = {}
        value_cols = {}

        for operator in self.dim_dict:
            self.shap_values_arrays[operator] = []
            shap_values_arrays_sum[operator] = []
            value_cols[operator] = []
        ...
        for operator in self.dim_dict:
            if self.shap_values_arrays[operator] == []:
                continue
            shap_values_arrays_sum[operator] = np.array(self.shap_values_arrays[operator]).sum(axis=0).tolist()
            for i, val in enumerate(shap_values_arrays_sum[operator]):
                if val > 0:
                    value_cols[operator].append(i)

        with open(self.save_dir + "/" + self.filter_type + "_values_array.pickle", "wb") as f:
            pickle.dump(value_cols, f)
    ...
    def load_config(self, epoch):
        for name in self.units:
            save_filename = '%s_net_%s.pth' % (epoch, name)
            path_array = self.save_dir.split('/')
            path_array[3] = 'knob_model'

            save_path = os.path.join('/'.join(path_array), save_filename)
            if not os.path.exists(save_path):
                raise ValueError("model {} doesn't exist".format(save_path))
    ...
    def load_transfer_config(self, epoch):
        for name in self.units:
            save_filename = '%s_net_%s.pth' % (epoch, name)
            path_array = self.save_dir.split('/')
            path_array[3] = 'knob_modelchange'

...
from RDBLab.dataset.oltp_dataset.attr_rel_dict import all_dicts
...
class OLTPDataset():
    def __init__(self, opt):
        ...
        if opt.new_mid_data:
            ...
            with open(mid_data_dir + '/serialize_mean_range_dict.pickle', 'wb') as f:
            ...
            with open(mid_data_dir + '/serialize_mean_range_dict.pickle', 'rb') as f:
    ...
    def make_dataset(self, tables, predicates, labels):
        ...
        print(table_tensors.shape)
        print(predicate_tensors.shape)
    ...
    def normalize(self):  # compute the mean and std vec of each operator
        ...
        def parse_input(data):
            feat_vec = np.array(
                [np.hstack(
                    (self.input_func[jss["Node Type"]](jss)))
                    for jss in data])

...
def tpch_dim(dic_factor):
    tpch_dim_dict = {'Seq Scan': num_rel + max_num_attr * 3 + basics + len(dic_factor["Seq Scan"]),
                     ...
                     'Bitmap Index Scan': num_index + basics + len(dic_factor["Bitmap Index Scan"]),
                     'Sort': basics + len(sort_algos) + 128 + 32 + len(dic_factor["Sort"]),
    ...
    def __init__(self, opt):
        ...
        if opt.new_mid_data:
            ...
            for i in range(self.num_q):
                fname = opt.data_dir + "/" + dir + "/q" + str(i + 1) + ".json"

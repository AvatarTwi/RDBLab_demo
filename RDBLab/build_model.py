...
from RDBLab.models.GBDT import GradientBoosting
from RDBLab.models.LinearRegress import Linear_Regress
from RDBLab.models.MLP import MLP
from RDBLab.models.MSCN import MSCNModel
from RDBLab.models.Netv2 import Net_v2
from RDBLab.models.QPPNet import QPPNet
from RDBLab.models.RandomForest import RandomForest
from RDBLab.utils.opt_parser import save_opt
...
def build_md(dataset, model_type, opt, dim_dict):
    ...
    FUNCTION = {
        ...
        "config_eval": model.calculate_config,
        "transfer_eval": model.calculate_config,
        # "imp_eval":model.calculate_imp,
        # "shap4net_eval":model.calculate_shap_for_each
    }
    ...
    if opt.mode == 'eval':
        ...
        print('eval:', RDBLab.config.total)
        eval_log.write(
            "Total Eval Time: " + str(float(round((end_eval_time - start_eval_time) * 1000000 - RDBLab.config.total))) + "us")
        eval_log.close()
    ...
    if opt.mode == 'mobility_eval':
    ...
    elif opt.mode == 'config_eval':
        function = FUNCTION[opt.mode]

        model.load_config("best")

        return function(dataset.test_dataset)

    elif opt.mode == 'transfer_eval':
        function = FUNCTION[opt.mode]

        model.load_transfer_config("best")
        ...
        eval_log.write('total_loss: {}; test_loss: {}; pred_err: {}; ' \
                       .format(model.last_total_loss, model.last_test_loss,
                               model.last_pred_err))
        print('train:', RDBLab.config.total)
        print("running time:", float(round((end_time - start_time) * 1000000) / 1000000))
        eval_log.write("\nTotal Running Time: " + str(
            float(round((end_time - start_time) * 1000000 - RDBLab.config.total) / 1000000)) + "s")
        print("final running time:", float(round((end_time - start_time) * 1000000 - RDBLab.config.total) / 1000000))
        RDBLab.config.total = 0.0
        eval_log.close()
        ...
        eval_log.write(
            "Total Eval Time: " + str(float(round((end_eval_time - start_eval_time) * 1000000 - RDBLab.config.total))) + "us")
        print("final eval time:", float(round((end_eval_time - start_eval_time) * 1000000 - RDBLab.config.total) / 1000000))
        eval_log.close()
        ...
        eval_log.write(
            ...
            str(float(round((end_eval_time - start_eval_time) * 1000000 - RDBLab.config.total))) + "us")
        eval_log.close()
